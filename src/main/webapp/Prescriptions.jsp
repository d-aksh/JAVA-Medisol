...
<body>
  ...
  <section class="container-fluid bg-info mt-4 p-4">
  	...
  	%>
  		...
  		<a target="_blank" href="GetPrescriptionImage?id=<%=prescription.get("id")%>"><img height="100px" src="GetPrescriptionImage?id=<%=prescription.get("id")%>"></a>
  		<p class="p-2 m-0">
	     Email: <b><%=prescription.get("email")%></b>
	     Status: <b><%=prescription.get("status")%></b>
	     <%
		 String status=(String)prescription.get("status");
		 if(status.equalsIgnoreCase("pending")){
		 %>
	     <a class="btn btn-sm btn-danger" href="ChangePrescriptionStatus?id=<%=prescription.get("id") %>&status=Done">Done</a>
	     <form action="CartForUser.jsp" method="post">
	     	<input type="hidden" name="uemail" value="<%=prescription.get("email")%>" />
	     	<button class="btn btn-sm btn-success">Add Items to Cart</button>
	     </form>
	     <%
		 }
		 %>
