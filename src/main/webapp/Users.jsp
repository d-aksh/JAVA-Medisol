...
<body>
  ...
  <section class="container-fluid bg-info mt-4 p-4">
  	...
  	%>
	     ...
	     Status: <b><%=user.get("status")%></b>
	     <%
		 String status=(String)user.get("status");
		 if(status.equalsIgnoreCase("active")){
		 %>
	     <a class="btn btn-sm btn-danger" href="ChangeUserStatus?email=<%=user.get("email") %>&status=Deactive">Deactive</a>
	     <%
		 }else{
		 %>
	     <a class="btn btn-sm btn-success" href="ChangeUserStatus?email=<%=user.get("email") %>&status=Active">Active</a>
	     <%	 
		 }
		 %>
	     </p>
